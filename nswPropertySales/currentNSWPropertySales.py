...
class RecordAddrSales(Extractor):
    ...
    def check_len_args(self, args):
        if (len(args) != self.length_args):
            warnings.warn(
                "lenght of argument is not correct for Record Address Sales")
            return False
        return True
...
class RecordDesc(Extractor):
    ...
    def check_len_args(self, args):
        if (len(args) < self.length_args):
            warnings.warn(
                "lenght of argument is not correct for Record Description")
            return False
        return True
        ...
        match record_type:
            ...
            case "B":
                ...
                if propertyHeader and propertyAddrSales and propertyDesc and propertyOwner:
                    propertyList.append(NSWPropertyInfo(
                        propertyHeader, propertyAddrSales, propertyDesc, propertyOwner))
    ...
    for rawResult in propertyList:
        ...
        data = Property_Data_Type(
            ...
            download_date=datetime.strptime(
                addressSales.download_date, "%Y%m%d %H:%M").isoformat().split("T")[0],
